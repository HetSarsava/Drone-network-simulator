function drone_simulation()
    ...
    hSlider = uicontrol('Style', 'slider', 'Min', 0.0, 'Max', 0.2, 'Value', 0.05, ...
        'Position', [20 20 200 20]);
    uicontrol('Style','text', 'Position',[20 45 200 20], ...
        'String','Speed Adjustment (Delay)', ...
    ...
    while ishandle(fig)
            ...
            switch d.state
                case 'CHARGING'
                    ...
                    if d.timer <= 0
                        ...
                        % 1. Release Spot logic: Manage Queue for THIS station
                        ...
                        if ~isempty(waiting_indices)
                            ...
                            % Promote the selected drone to CHARGING
                            if promote_idx > 0
                        ...
                        if ~isempty(neighbors)
                            ...
                            pad_occupancy(d.station, d.pad) = 0; % Free the physical pad
                            d.pad = 0;
                ...
                case 'LANDING'
                    if reached
                        ...
                        if active_chargers < 3
                            ...
                            d.state = 'WAITING';
                            d.timer = 0; % No timer, waits for signal
                        end
                    end
                    
                case 'WAITING'
                    % Do nothing. Position is fixed.
                    % State change happens when another drone leaves 'CHARGING'
            end
        ...
        % Safety check: Stop if window/slider is closed
        if ~ishandle(hSlider)
            break;
        end

        % Get slider value for delay
...
function update_drone_gfx(gfx, d, station_h)
    ...
    if strcmp(d.state, 'WAITING'), status_col = 'y'; end % Yellow for waiting
...
function draw_station_building(pos, w, h, name)
    ...
    for i = 1:6
        ...
        % Color code: 1-3 Green (Chargers), 4-6 Yellow (Waiting spots)
        if i <= 3
            p_color = [0.2 0.8 0.2]; % Greenish
        else
            p_color = [0.8 0.7 0.1]; % Yellowish
        end
